...
The Windows 8/8.1 edition of WinDeploy uses the following separately licensed components. They remain the property of their respective authors.
...
WinDeploy dynamically loads `libwim-15.dll`; the library files are kept separate so recipients can replace them with a compatible build as permitted by the LGPL.
