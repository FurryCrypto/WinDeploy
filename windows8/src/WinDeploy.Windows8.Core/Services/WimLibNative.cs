...
internal static class WimLibNative
{
    ...
    public static void EnsureInitialized()
    {
        lock (Sync)
        {
            ...
            if (!File.Exists(path))
                throw new WinDeployException("ErrorImageOpen", "The imaging library is missing: " + path);
            ...
            { throw new WinDeployException("ErrorImageOpen", "The imaging library could not be initialized.", exception); }
            _initialized = true;
        }
    }
    ...
    internal static ushort ReadPeMachine(string path)
    {
        ...
        catch (WinDeployException)
        {
            throw;
        }
        catch (Exception exception)
        {
            throw new WinDeployException("ErrorImageOpen", "The imaging library is not a valid Windows binary: " + path, exception);
        }
    }
}
