...
public sealed class ExecutionPlanValidator
{
    ...
    public async Task ValidateAsync(InstallationPlan plan, CancellationToken cancellationToken = default)
    {
        ...
        if (plan.BypassWindows11Requirements && plan.Generation != WindowsGeneration.Windows11)
            throw new WinDeployException("ValidationPlanUnreadable", "The Windows 11 bypass was selected for another version.");
        ...
        if (disk == null) throw new WinDeployException("ValidationDiskMissing", plan.DestinationDisk.DiskNumber.ToString());
        ...
        if (!DiskMatches(disk, plan.DestinationDisk))
            throw new WinDeployException("ValidationDiskChanged", disk.SafeDisplayName);
        var destination = FindPartition(disk, plan.DestinationPartition);
        if (destination == null) throw new WinDeployException("ValidationPartitionChanged", "Destination partition identity did not match.");
        ...
        if (boot == null) throw new WinDeployException("ValidationBootPartitionChanged", "Boot partition identity did not match.");
