...
public sealed class WimService
{
    ...
    {
        if (!File.Exists(path)) throw new WinDeployException("ErrorImageNotFound", path);
        ...
        catch (WinDeployException) { throw; }
        catch (Exception exception)
        { throw new WinDeployException("ErrorImageMetadata", exception.Message, exception); }
    }
    ...
    internal static IReadOnlyList<WindowsImageEdition> ParseEditions(string xml)
    {
        ...
        if (images.Length == 0) throw new WinDeployException("ErrorNoEditions", "The image contains no indexes.");
