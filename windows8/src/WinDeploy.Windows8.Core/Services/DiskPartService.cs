...
public sealed class DiskPartService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!actual.FileSystem.Equals("NTFS", StringComparison.OrdinalIgnoreCase))
            throw new WinDeployException("ErrorFormatDestination", "The destination did not report NTFS after formatting.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!actual.DriveLetter.HasValue || actual.DriveLetter.Value != letter)
            throw new WinDeployException("ErrorBootPartitionAccess", "DiskPart did not assign the requested boot access path.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (disk == null || disk.SizeBytes != plan.DestinationDisk.SizeBytes ||
            ...
            throw new WinDeployException("ValidationDiskChanged", plan.DestinationDisk.Model);
        ...
        if (partition == null) throw new WinDeployException("ValidationPartitionChanged", expected.PartitionNumber.ToString());
        if (expected == plan.DestinationPartition && (partition.IsProtected || partition.IsBitLocker))
            throw new WinDeployException("ValidationProtectedPartition", partition.StableKey);
        return partition;
    }
    ...
    private async Task RunScriptAsync(IEnumerable<string> commands, InstallationLog log,
        CancellationToken cancellationToken)
    {
        var scriptPath = Path.Combine(Path.GetTempPath(), "WinDeployWindows8-" + Guid.NewGuid().ToString("N") + ".txt");
        try
        {
            ...
            if (!result.Succeeded || combined.IndexOf("DiskPart has encountered an error", StringComparison.OrdinalIgnoreCase) >= 0 ||
                ...
                throw new WinDeployException("ErrorFormatDestination",
    ...
    private static char FindFreeLetter(char preferred)
    {
        ...
        throw new WinDeployException("ErrorBootPartitionAccess", "No free drive letter is available.");
    }
}
