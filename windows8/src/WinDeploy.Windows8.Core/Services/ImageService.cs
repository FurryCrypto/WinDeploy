...
public sealed class ImageService : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        switch (Path.GetExtension(sourcePath).ToLowerInvariant())
        {
            ...
            default: throw new WinDeployException("ErrorUnsupportedImageType", Path.GetExtension(sourcePath));
        }
    }
    ...
    private static DiscFileSystem OpenIsoFileSystem(Stream stream)
    {
        ...
        { throw new WinDeployException("ErrorIsoMount", "The ISO/UDF filesystem could not be read.", exception); }
        throw new WinDeployException("ErrorIsoMount", "The file does not contain a readable ISO 9660 or UDF filesystem.");
    }
