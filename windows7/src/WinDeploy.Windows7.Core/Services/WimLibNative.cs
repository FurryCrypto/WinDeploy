...
internal static class WimLibNative
{
    ...
    public static void EnsureInitialized()
    {
        lock (Sync)
        {
            ...
            if (!File.Exists(path))
                throw new WinDeployException("ErrorImageOpen", "The Windows 7 imaging library is missing: " + path);
            try { Wim.GlobalInit(path, InitFlags.None); }
            catch (Exception exception)
            { throw new WinDeployException("ErrorImageOpen", "The Windows 7 imaging library could not be initialized.", exception); }
            _initialized = true;
        }
    }
}
