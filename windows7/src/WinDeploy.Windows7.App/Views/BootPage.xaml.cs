...
namespace WinDeploy.Windows7.Views;
...
public partial class BootPage : Page
{
    ...
    private void UpdateResult(PlanValidationResult result)
    {
        ...
        Issues.ItemsSource = result.Issues.Select(x => new IssueItem(App.Services.Localizer.Get(x.MessageKey),
            ShellIconService.Get(x.Severity == PlanSeverity.Error ? StockIconId.Error : x.Severity == PlanSeverity.Warning ? StockIconId.Warning : StockIconId.Information))).ToArray();
    ...
    private sealed class IssueItem { public IssueItem(string text, ImageSource icon) { Text = text; Icon = icon; } public string Text { get; } public ImageSource Icon { get; } }
}
