...
public sealed class WimService
{
    ...
    private static IReadOnlyList<WindowsImageEdition> ReadEditions(string path, CancellationToken cancellationToken)
    {
        if (!File.Exists(path))
        {
            throw new ESDInstallerException("ErrorImageNotFound", path);
        }

        var handle = WIMCreateFile(path, WimGenericRead, WimOpenExisting, 0, 0, out _);
        if (handle == IntPtr.Zero || handle == new IntPtr(-1))
        {
            throw new ESDInstallerException("ErrorImageOpen", $"WIMCreateFile failed with Win32 error {Marshal.GetLastWin32Error()}.");
        }
        ...
        try
        {
            ...
            if (!WIMSetTemporaryPath(handle, scratchPath))
            {
                ...
                throw new ESDInstallerException("ErrorImageScratch",
            ...
            if (count == 0)
            {
                throw new ESDInstallerException("ErrorNoEditions", "The image contains no indexes.");
            }
            ...
            for (var index = 1; index <= count; index++)
            {
                ...
                {
                    throw new ESDInstallerException("ErrorImageMetadata", $"WIMLoadImage({index}) failed with Win32 error {Marshal.GetLastWin32Error()}.");
                }
                ...
                try
                {
                    ...
                    {
                        throw new ESDInstallerException("ErrorImageMetadata", $"WIMGetImageInformation({index}) failed with Win32 error {Marshal.GetLastWin32Error()}.");
                    }
    ...
    internal static WindowsImageEdition ParseEdition(string xml, int fallbackIndex)
    {
        ...
        if (image is null)
        {
            throw new ESDInstallerException("ErrorImageMetadata", "The WIM metadata did not contain an IMAGE element.");
        }
    ...
    internal static string EnsureScratchDirectory()
    {
        ...
        {
            throw new ESDInstallerException("ErrorImageScratch", "The local application data directory is unavailable.");
        }

        var scratchPath = Path.Combine(localAppData, "ESDInstaller", "WimScratch");
        try
        {
        ...
        {
            throw new ESDInstallerException("ErrorImageScratch", $"Scratch path is not writable: {scratchPath}", exception);
        }
    }
