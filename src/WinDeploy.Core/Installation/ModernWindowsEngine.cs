...
public sealed class ModernWindowsEngine : IInstallationEngine
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (!PrivilegeService.IsAdministrator())
            throw new WinDeployException("AdministratorRequired", "The elevated worker is not running as administrator.");
        ...
        if (!apply.Succeeded)
            throw new WinDeployException("ErrorDismApply", $"DISM exited with code {apply.ExitCode}. {apply.StandardError}".Trim());
        ...
        try
        {
            ...
            if (!bootResult.Succeeded)
                throw new WinDeployException("ErrorBcdBoot", $"BCDBoot exited with code {bootResult.ExitCode}. {bootResult.StandardError}".Trim());
        ...
        InstallationExecutionContext context, CancellationToken cancellationToken)
    {
        if (plan.Generation != WindowsGeneration.Windows11)
            throw new WinDeployException("ValidationPlanUnreadable", "Unsupported-hardware bypass requires a Windows 11 image.");

        var systemHive = Path.Combine(destinationRoot, "Windows", "System32", "Config", "SYSTEM");
        if (!File.Exists(systemHive))
            throw new WinDeployException("ErrorWindows11Bypass", $"Offline SYSTEM hive was not found: {systemHive}");

        var reg = Path.Combine(Environment.SystemDirectory, "reg.exe");
        var mountName = $"WinDeploy_{plan.PlanId:N}";
        ...
        try
        {
                     ...
                     })
            {
                if (!verification.StandardOutput.Contains(valueName, StringComparison.OrdinalIgnoreCase))
                    throw new WinDeployException("ErrorWindows11Bypass", $"Offline registry verification failed for {valueName}.");
            }
        ...
        finally
        {
            if (loaded)
            {
                ...
                if (!unload.Succeeded)
                    throw new WinDeployException("ErrorWindows11Bypass", $"The offline SYSTEM registry hive could not be unloaded. {unload.StandardError}".Trim());
            }
        }
    }
        ...
        InstallationExecutionContext context, CancellationToken cancellationToken)
    {
        ...
        if (!result.Succeeded)
            throw new WinDeployException("ErrorWindows11Bypass",
        ...
        InstallationExecutionContext context, CancellationToken cancellationToken)
    {
        ...
        if (!result.Succeeded) throw new WinDeployException("ErrorFormatDestination", result.StandardError.Trim());
        var info = DeserializeJson<AccessResult>(result.StandardOutput);
        if (info is null || string.IsNullOrWhiteSpace(info.Root))
            throw new WinDeployException("ErrorFormatDestination", "The formatting operation did not return an access path.");
        return info.Root;
    }
        ...
        InstallationExecutionContext context, CancellationToken cancellationToken)
    {
        ...
        var result = await context.Processes.RunPowerShellAsync(script, environment,
            ...
            cancellationToken: cancellationToken).ConfigureAwait(false);
        if (!result.Succeeded) throw new WinDeployException("ErrorBootPartitionAccess", result.StandardError.Trim());
        return DeserializeJson<AccessResult>(result.StandardOutput)
               ?? throw new WinDeployException("ErrorBootPartitionAccess", "No boot access path was returned.");
    }
    ...
    private static void VerifyFiles(InstallationPlan plan, string destinationRoot, string bootRoot)
    {
        ...
        if (plan.FirmwareMode == FirmwareMode.Uefi)
        {
            ...
            if (!File.Exists(fallback)) throw new WinDeployException("ErrorBootFallbackVerification", fallback);
        }
        else
        {
