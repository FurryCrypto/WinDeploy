...
public sealed class ExecutionPlanValidator
{
    ...
    public async Task ValidateAsync(InstallationPlan plan, CancellationToken cancellationToken = default)
    {
        if (plan.Engine != InstallationEngineKind.ModernWindows)
            throw new WinDeployException("LegacyEngineUnavailable", plan.Engine.ToString());
        if (plan.BypassWindows11Requirements && plan.Generation != WindowsGeneration.Windows11)
            throw new WinDeployException("ValidationPlanUnreadable", "The Windows 11 bypass was set for a non-Windows 11 image.");
        if (!File.Exists(plan.Source.SourcePath) || !File.Exists(plan.Source.ImagePath))
            throw new WinDeployException("ErrorImageNotFound", plan.Source.SourcePath);
        ...
        if (sourceInfo.Length != plan.Source.SourceSizeBytes || sourceInfo.LastWriteTimeUtc != plan.Source.SourceLastWriteUtc)
            throw new WinDeployException("ValidationSourceChanged", plan.Source.SourcePath);
        ...
        if (edition is null || edition.Build != plan.Edition.Build || edition.Architecture != plan.Edition.Architecture)
            throw new WinDeployException("ValidationEditionChanged", plan.Edition.Index.ToString());
        ...
        if (disk is null) throw new WinDeployException("ValidationDiskMissing", plan.DestinationDisk.DiskNumber.ToString());
        if (string.IsNullOrWhiteSpace(plan.DestinationDisk.UniqueId) && string.IsNullOrWhiteSpace(plan.DestinationDisk.SerialNumber))
            throw new WinDeployException("ValidationDiskIdentityUnavailable", plan.DestinationDisk.Model);
        if (!DiskMatches(disk, plan.DestinationDisk))
            throw new WinDeployException("ValidationDiskChanged", $"Expected {plan.DestinationDisk.UniqueId}; found {disk.UniqueId}");

        var destination = FindPartition(disk, plan.DestinationPartition);
        if (destination is null) throw new WinDeployException("ValidationPartitionChanged", "Destination partition identity did not match.");
        if (destination.IsProtected || destination.IsBitLocker)
            throw new WinDeployException("ValidationProtectedPartition", destination.StableKey);
        if (destination.PartitionNumber <= 0 || destination.Role != PartitionRole.BasicData)
            throw new WinDeployException("ValidationDestinationMustBeBasicData", destination.StableKey);

        var boot = FindPartition(disk, plan.BootPartition);
        if (boot is null) throw new WinDeployException("ValidationBootPartitionChanged", "Boot partition identity did not match.");
        if (plan.FirmwareMode == FirmwareMode.Uefi && boot.Role != PartitionRole.EfiSystem)
            throw new WinDeployException("ValidationEfiPartitionRequired", boot.StableKey);
        if (plan.FirmwareMode == FirmwareMode.Bios && !boot.IsActive)
            throw new WinDeployException("ValidationActivePartitionRequired", boot.StableKey);
        if (plan.FirmwareMode == FirmwareMode.Uefi && disk.PartitionScheme != PartitionScheme.Gpt ||
            ...
            throw new WinDeployException("ValidationFirmwareSchemeChanged", $"{plan.FirmwareMode}/{disk.PartitionScheme}");
    }
