...
public sealed class DiskService
{
    ...
    public async Task<IReadOnlyList<DiskInfo>> GetDisksAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!result.Succeeded)
            throw new WinDeployException("ErrorDiskEnumeration", result.StandardError.Trim());
        ...
        catch (Exception exception) when (exception is not WinDeployException)
        {
            throw new WinDeployException("ErrorDiskEnumeration", result.StandardOutput, exception);
        }
    }
