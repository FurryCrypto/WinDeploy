...
public sealed class ImageService : IAsyncDisposable
{
    ...
    public async Task<WindowsImage> InspectAsync(string sourcePath, CancellationToken cancellationToken = default)
    {
        ...
        if (!File.Exists(sourcePath)) throw new WinDeployException("ErrorImageNotFound", sourcePath);
        ...
        return extension switch
        {
            ...
            _ => throw new WinDeployException("ErrorUnsupportedImageType", extension)
        };
    }
    ...
    private async Task<WindowsImage> InspectIsoAsync(string sourcePath, CancellationToken cancellationToken)
    {
        ...
        var result = await _processRunner.RunPowerShellAsync(script,
            new Dictionary<string, string?> { ["WINDEPLOY_IMAGE_PATH"] = sourcePath },
            cancellationToken: cancellationToken).ConfigureAwait(false);
        if (!result.Succeeded)
        {
            throw new WinDeployException("ErrorIsoMount", result.StandardError.Trim());
        }

        IsoMountResult? mount;
        try { mount = JsonSerializer.Deserialize<IsoMountResult>(ExtractJson(result.StandardOutput)); }
        catch (Exception exception) { throw new WinDeployException("ErrorIsoMount", result.StandardOutput, exception); }
        if (mount is null || string.IsNullOrWhiteSpace(mount.Root))
            throw new WinDeployException("ErrorIsoMount", "PowerShell did not return a mount root.");
        ...
        if (File.Exists(Path.Combine(mount.Root, "sources", "install.swm")))
            throw new WinDeployException("ErrorSplitWimUnsupported", "The ISO contains a split install.swm image.");
        throw new WinDeployException("ErrorNoInstallImage", mount.Root);
    }
    ...
    {
        const string script = "$ErrorActionPreference='SilentlyContinue'; Dismount-DiskImage -ImagePath $env:WINDEPLOY_IMAGE_PATH";
        foreach (var path in _mountedByThisProcess.ToArray())
        {
            await _processRunner.RunPowerShellAsync(script,
                new Dictionary<string, string?> { ["WINDEPLOY_IMAGE_PATH"] = path }).ConfigureAwait(false);
        }
