...
public sealed class WorkerClient
{
    ...
    {
        var localRoot = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "WinDeploy");
        ...
        var pipeName = $"WinDeploy-{plan.PlanId:N}";
        ...
        var workerPath = Path.Combine(workerDirectory, "WinDeploy.Worker.exe");
        if (!File.Exists(workerPath))
        {
            workerDirectory = AppContext.BaseDirectory;
            workerPath = Path.Combine(workerDirectory, "WinDeploy.Worker.exe");
        }
        if (!File.Exists(workerPath))
            throw new WinDeployException("ErrorWorkerMissing", workerPath);
        ...
        try
        {
            ...
            if (worker is null) throw new WinDeployException("ErrorWorkerStart", workerPath);
